...
10. _User wants to see when content of a page has changed:_ Grouping on digest (hast of content) and displaying the first result for each group does this. `query=url:"example.com/kittens.html & group=true & group.field=digest & sort=crawl_date asc & fl=crawl_date`.
...
14. _Bulk/batch requests:_ Solr 4 supports paging and Solr 5 supports streaming exports; see [Exporting Result sets](Bulk/batch requests). One aber dabei is that parallel requests of chunks is tricky. That might require some intemediate step where the cursorMarks are determined up front.
